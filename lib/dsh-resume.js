...
function slashReferenceFromUserText(text, provider) {
	...
	if (!ref || ref.toLowerCase() === "latest") return;
	return ref;
}
...
function slashReferenceFromSession(events, provider) {
	...
	for (let index = events.length - 1; index >= 0; index -= 1) {
		...
		if (new RegExp(`/resume-${provider}\\b`, "i").test(text)) return;
	}
}
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const inferred = args.reference?.trim() || (action === "show" ? slashReferenceFromSession(exec.agent?.session.events, args.provider) : void 0);
